...
class EDVRNet(BaseModule):
    ...
    def forward(self, lq):
        ...
        for i in range(t):
            ...
            if i == 0:
                x = torch.stack((lq[:, i], lq[:, i], lq[:, i], lq[:, i + 1], lq[:, i + 2]),
                                 dim=1)
            elif i == 1:
                x = torch.stack(
                    (lq[:, i - 1], lq[:, i - 1], lq[:, i], lq[:, i + 1], lq[:, i + 2]), dim=1)
            elif i == t - 2:
                x = torch.stack(
                    (lq[:, i - 2], lq[:, i - 1], lq[:, i], lq[:, i + 1], lq[:, i + 1]), dim=1)
            elif i == t - 1:
                x = torch.stack((lq[:, i - 2], lq[:, i - 1], lq[:, i], lq[:, i], lq[:, i]),
                                 dim=1)
            else:
                x = lq[:, i - 2: i + 3]
            n, t, c, h, w = x.size()
        ...
        SR = torch.stack(SR_list, dim=1)
        return SR

    def init_weights(self):
        """Init weights for weights."""
